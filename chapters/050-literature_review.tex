...
Similar to MCTChain the paper \citetitle{Nikodem20}~\cite{Nikodem20} introduces a decentralized tracking algorithm following the TbD paradigm that on the one hand performs intra-camera tracking locally on the camera and on the other hand uses an ISM-based wireless device-to-device communication for inter-camera tracking.
